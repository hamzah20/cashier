    ...
    switch ($role) {
        case'LOGIN_KASIR': 
            ...
            $password = md5($_POST['password']); 
             ...
             $sqlemail="SELECT COUNT(*) AS TOTAL_USER FROM user where (id_user='".$username."' or username='".$username."') and password='".$password."' and user_group='Kasir' ";

            $r_email    = mysqli_query($conn,$sqlemail);
            $rs_email   = mysqli_fetch_array($r_email);

            if($rs_email['TOTAL_USER'] > 0){
                 $sql="SELECT * FROM v_s_user where (id_user='".$username."' or username='".$username."') and password='".$password."' and user_group='Kasir'";
